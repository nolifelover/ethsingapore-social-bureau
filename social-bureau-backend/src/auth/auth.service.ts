  ...
  async authenticationWithPublicAddress(payload: AuthenticationPubliAddressInput) {
    ...
    if (user) {
      ...
      if (address.toLowerCase() === publicAddress.toLowerCase()) {
        ...
        const isInspector = await this.contractService.isInspector(address);
        Logger.debug(`check is inspector = ${isInspector}`);
        if(isInspector && isInspector === 'true' || isInspector == true){
          user.role = Role.INSPECTOR;
          this.userService.updateRole(user.uid, Role.INSPECTOR);
        }else{
          this.userService.updateRole(user.uid, Role.USER);
        }
